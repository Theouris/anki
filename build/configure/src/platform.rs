...
/// Usually None to use the host architecture.
///
/// On Windows ARM64, allow an opt-in native ARM64 build with WIN_ARM64=1.
/// Otherwise keep the historical Windows x64 behavior.
///
/// On non-Windows platforms, use the host architecture unless overridden.
pub fn overriden_python_venv_platform() -> Option<Platform> {
    if cfg!(target_os = "windows") {
        if env::var("WIN_ARM64").ok().as_deref() == Some("1") {
            Some(Platform::WindowsArm)
        } else {
...
/// If WIN_ARM64 is set, an ARM64 wheel will be built on Windows ARM64.
pub fn overriden_python_wheel_platform() -> Option<Platform> {
    ...
    } else if env::var("WIN_ARM64").ok().as_deref() == Some("1") {
        Some(Platform::WindowsArm)
    } else {
